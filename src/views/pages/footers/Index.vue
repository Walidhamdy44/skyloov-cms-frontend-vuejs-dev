...
const getPages = () => {
    ...
    window.axios
        ...
        .then((response) => {
            // Store all footer pages sorted by newest first
            allPagesData.value = response.data.data
                .filter((page) => page.slug.startsWith('footer'))
            ...
            <div class="card">
                <Toolbar class="mb-4">
                    <template v-slot:start>
                        <div class="my-2">
                            <Button label="New" icon="pi pi-plus" class="p-button-success mr-2" @click="router.push({ name: 'create-footers' })" />
                ...
                <div v-if="!loading && pages && pages.length === 0" class="text-center p-4">
                    <p class="text-gray-500 text-lg font-medium">No footers exist.</p>
                    <p class="text-gray-400">Click the <strong>"New"</strong> button above to create your first footer.</p>
                </div>
                    ...
                    >
                        <template #header>
                            <div class="flex flex-column md:flex-row md:justify-content-between md:align-items-center">
                                <h5 class="m-0">Manage Footers</h5>
                            </div>
                        </template>
                        ...
                        <Column headerStyle="min-width:10rem;">
                            <template #body="slotProps">
                                <div class="actions">
                                    <Button icon="pi pi-folder" class="p-button-rounded p-button-info mr-2" @click="router.push({ name: 'manage-footers-blocks', params: { id: slotProps.data.id } })" />
                                    <Button icon="pi pi-pencil" class="p-button-rounded p-button-success mr-2" @click="router.push({ name: 'edit-footers', params: { id: slotProps.data.id } })" />
